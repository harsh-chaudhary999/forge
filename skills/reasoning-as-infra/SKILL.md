...
Write to `~/forge/brain/prds/<task-id>/council/infra.md`. A full worked example
(`# Infra Perspective` — Database / Caching / Events / Search / Monitoring with schema
DDL, Kafka topics + schemas, ES mappings, and metrics/alerts/SLOs) is in
...
This skill's reference-grade catalogs live in `reference/` — read the one you need:

- Edge cases (1–9), failure scenarios, and common pitfalls (1–7): [reference/edge-cases.md](reference/edge-cases.md)
- Scaling decision tree (bottlenecks, vertical/horizontal, partitioning, pool sizing, TTL tuning): [reference/scaling.md](reference/scaling.md)
- Failure scenario handbook (metrics-to-watch + immediate action + recovery + prevention for DB / cache / event bus / search): [reference/failure-handbook.md](reference/failure-handbook.md)
- Consistency model decision tree (strong / causal / eventual / weak, cache TTL by model, per-feature choice): [reference/consistency.md](reference/consistency.md)
- Deployment & migration patterns (1–6: zero-downtime migration, blue-green reindex, feature-flag rollout, canary, Kafka consumer upgrade, DB failover/switchback): [reference/deployment.md](reference/deployment.md)
