...
**Assistant-led messaging (repo-wide — HARD-NORM):** In **chat**, the **title / first paragraph / top `##`** must name **only** the **first failing row** above — e.g. *Next: **`qa-manual-test-cases-from-prd`** / **`manual-test-cases.csv`*** when row **3** fails. **Do not** open with **`## What unlocks eval YAML`**, *Per Step −1… eval `*.yaml`* **as the main heading**, or a **YAML-first** essay when **`manual-test-cases.csv`** is the blocker; Step −1 is **documentation order**, not the **H1** of the message. **Allowed:** one **subordinate** sentence after the CSV call-to-action (*after CSV approval, eval YAML follows*). **`docs/forge-one-step-horizon.md`** — **Headline = immediate next step**.
