...
**AI hosts (dialogue):** When eliciting answers in chat, follow **[`docs/forge-one-step-horizon.md`](docs/forge-one-step-horizon.md)** — **one-step horizon** (only the immediate next prerequisite unless the user asks for the full roadmap) and **no bundled intake** (do not combine one **`AskQuestion`** for task-id with prose-only demands for Q9 design locks **plus** downstream QA/YAML copy in the same message — same doc, section **Bundled intake turns**). Full pipeline order remains in this README and **`commands/`**; reference docs may list the chain; do not restate it every turn in session.
