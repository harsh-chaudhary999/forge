...
## Headline = immediate next step (not downstream eval YAML)

**Problem:** The assistant uses **`## What unlocks eval YAML`**, *Per **`qa-write-scenarios` Step −1*** **eval `*.yaml`***, or the **full QA→CSV→YAML chain** as the **title**, **`##` heading**, or **first paragraph** of a message — while the user’s **actual** next work is **`qa-manual-test-cases-from-prd`**, **`manual-test-cases.csv`**, or expanding rows — **not** writing YAML yet. **Eval YAML** is **later** in the ordering table; it must **not** **steal the headline** when it is **not** the **immediate** gate.

**Required:** The **top-of-message** framing (first heading, bold lead, or opening sentence) names the **single next** artifact or skill from **`qa-write-scenarios` Step −1** **first failing row** — e.g. **Next: complete / expand `qa/manual-test-cases.csv`** (**`qa-manual-test-cases-from-prd`**), or **Next: run `qa-prd-analysis`**. Step −1 is **reference** text in **`SKILL.md`** / **README** — **not** the **H1** of chat when CSV is still open.

**Forbidden:** Opening with *What unlocks eval YAML* / *YAML is gated by…* as the **primary** section when **`manual-test-cases.csv`** is **missing** or **needs work**. **Allowed:** one **subordinate** sentence after the CSV CTA (*after CSV approval, eval YAML follows Step −1*) — **not** before.

**Relation:** Extends **Horizon narration**, **No defensive downstream-gate narration**, **Stage-local questioning** — the **next step** is **CSV**, not YAML; narration must match.
