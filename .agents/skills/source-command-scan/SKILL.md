...
**Forge plugin:** Invokes **`scan-codebase`** / **`forge_scan.py`**; outputs go under **`~/forge/brain/products/<slug>/codebase/`** (per skill). Not a substitute for **`/forge`** delivery. **`scripts/install.sh`** (Cursor + Codex) copies the full **`tools/`** tree into the plugin dir so **`classes/`**, **`methods/`**, and the rest of the pipeline run from an installed plugin — not module-only stubs from a missing scanner.
...
- **Codex:** under **`$HOME/.Codex/plugins/cache/forge-plugin/forge/<version>/tools/forge_scan.py`** where **`<version>`** matches **`package.json`** in that directory (run **`ls ~/.Codex/plugins/cache/forge-plugin/forge/`** if unsure).
