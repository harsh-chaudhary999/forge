...
| "I'll prepend what **`/qa-write`** does and which gates are open before every confirm" | **Noise during elicitation.** Ask the question or give **one** line of context if needed — **`docs/forge-one-step-horizon.md`** **Elicitation mode**; **`using-forge`** item **7**. User can read **`commands/`** when they want the manual. |
