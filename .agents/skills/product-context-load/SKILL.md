...
This skill lists **`AskUserQuestion`** in **`allowed-tools`** — canonical for Codex and skill lint. Map to the host’s **blocking interactive prompt** per **`skills/using-forge/SKILL.md`** **Blocking interactive prompts** (Cursor **`AskQuestion`**; hosts without the tool: **numbered options + stop**). See **`using-forge`** **Interactive human input**.
...
6. **Load codebase scan if present**
   ...
   **Downstream routing (pass this forward in context):** Any phase that must name **concrete files** in product repos (council, tech-plan, semantic automation / drivers, design notes) should **take filenames and module boundaries from this scan output first** (`index.md`, `modules/*.md`, `api-surface.md`, hub lists in `SCAN.json`). Repo tree exploration is a **fallback** when scan is absent, stale, or a path is provably wrong after checking brain.
