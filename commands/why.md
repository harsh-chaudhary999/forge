...
The user’s argument is a **decision ID** (e.g. **`D20260410-001`**) or a **search term**. If nothing was provided, use a **blocking interactive prompt** per **`skills/using-forge/SKILL.md`** **Blocking interactive prompts** — e.g. **`AskUserQuestion`** or **numbered list** of recent decision IDs if you can list them from brain, else one field for ID or search term, then **stop** — not an open-ended *which decision?* with no structured reply path on hosts that need it.
