...
- **HARD-GATE — refuse silently dangerous dispatches:** If the conductor dispatch is for **feature / UI / non-test-only** work and **`~/forge/brain/prds/<task-id>/eval/`** does not exist or contains **zero** scenario files, **or** the handoff does not cite a logged **`[P4.0-EVAL-YAML]`** line, return **`BLOCKED_ORCHESTRATION`** to the conductor — **do not** write production code until State 4b is complete (`conductor-orchestrate` State 4b). When **`product.md`** sets **`forge_qa_csv_before_eval: true`**, also require **`~/forge/brain/prds/<task-id>/qa/manual-test-cases.csv`** with **≥1** row and a logged **`[P4.0-QA-CSV]`** before feature code. Test-only RED tasks from the same phase are OK if the prompt explicitly says **tests-first / TDD RED** only.
