...
**Default depth (MUST):** In the **same** `/plan` session, complete **silent exploration** (product repo **`Read`/`rg`**, brain **`codebase/`**, **`### 1b.2a`**, maximal §**1b** bodies) **without** asking the user “should I explore further?” — that is **not** optional depth. Only pause for **§0.1** / **§0.2** **judgment** questions in chat. If the model would otherwise stop at a short outline, **continue** until the write skill’s gates are met or **`BLOCKED`** with owner.

**Brevity split (MUST):** **Short** = **Section 0** + **chat** only. **§1b**, **`### 1b.2a`**, **Section 2** = **as verbose as completeness requires** — see **`tech-plan-write-per-project` §0.0**; do not compress elaboration for “readability.”
...
**Touchpoints & boundaries (MUST):** After §**1b.5** and **`#### 1b.5b`**, complete **`### 1b.2a`** — **full exploration mode** per **`tech-plan-write-per-project`**. Shallow **`### 1b.2a`** fails **`tech-plan-self-review`**. Cohort / adjacency / **`discovery-adjacency.md`**: **`docs/adjacency-and-cohorts.md`** + **`tech-plan-write-per-project` §0.1** rule 6.
