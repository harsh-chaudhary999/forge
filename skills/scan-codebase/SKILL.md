...
**Symptom:** Phase 1 produces thousands of source files; hub detection is skipped (>800 file threshold).
...
**Mitigation:**
1. Phase 1 already skips hub scoring when source count > 800 (produces no Tier 1/Tier 2 lists)
2. Phase 3 hub reads: without hub scores, read the top-level entry files (main.ts, app.ts, index.ts, server.go, main.py) as Tier 1 proxies — those are always the architectural hubs
3. Phase 4: run `phase4-brain-write.sh` as normal — it handles any number of files
4. For Phase 3 enrichment on very large repos (1000+ hub candidates): process in batches — read the first batch of hub files, write their brain nodes, then continue with the next batch. No file is skipped, just done in multiple passes.
5. Add `"monorepo": true` and `"hub_scoring_skipped": true` flags to SCAN.json
