...
**Location:** `.Codex/scripts/forge-worktree-cleanup.sh` (in each project)
...
bash .Codex/scripts/forge-worktree-cleanup.sh . 24 1

# Run from forge controller
for project in $(forge config projects); do
  bash "$project/.Codex/scripts/forge-worktree-cleanup.sh" "$project" 24 0
done
```
...
4. Run aggressive cleanup on projects: `bash .Codex/scripts/forge-worktree-cleanup.sh . 0 1` (stale_threshold = 0 hours, removes everything).
...
5. Run cleanup with explicit logging: `bash .Codex/scripts/forge-worktree-cleanup.sh . 24 1` (24-hour threshold, verbose output).
