...
# source-command-workspace

Use this skill when the user asks to run the migrated source command `workspace`.
...
```bash
FORGE_DIR=$(find ~/.Codex/plugins ~/.cursor/plugins ~/.cursor/plugins/local ~/.config/gemini/plugins \
...
```bash
FORGE_DIR="$(find ~/.cursor/plugins ~/.cursor/plugins/local ~/.Codex/plugins ~/.config/gemini/plugins \
  -path '*/forge/brain-template' -type d 2>/dev/null | head -1 | sed 's|/brain-template||')"
