...
5. **Speak only the immediate next dependency.** In **assistant messages** (not static README/docs), **do not** mention downstream process steps unless **(i)** they are the **very next** artifact/skill after the current step, **(ii)** the **current** prompt cannot be answered without naming them, or **(iii)** the user explicitly asked what comes later. Listing **`eval/*.yaml`**, **semantic eval** / **manifest** paths, CSV waivers, merge order, PR set, etc. **before** the user is on that gate **confuses** and reads like pressure — **forbidden**.
...
- **Machine eval:** **`~/forge/brain/prds/<task-id>/eval/*.yaml`** (declarative drivers; **`qa-write-scenarios`**, **`/qa-write`**, **`/qa`**) **and/or** **`qa/semantic-automation.csv`** + **`qa/semantic-eval-manifest.json`** (semantic path — **`docs/semantic-eval-csv.md`**). **`scenarios-manifest.md`**, **`branch-env-manifest.md`**, **`qa-run-report-<ts>.md`** come from the standalone QA pipeline when applicable.
