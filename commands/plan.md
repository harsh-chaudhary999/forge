...
<HARD-GATE>
Do NOT dispatch **`dev-implementer`** or open **Phase 4.1** from this command alone — tech plans are inputs to **State 4b** (QA CSV, semantic machine-eval manifest + CSV, RED) and then build. Per **`conductor-orchestrate`**, feature dispatch requires **`[P4.0-SEMANTIC-EVAL]`** (with valid artifacts per **`docs/forge-task-verification.md`**) and policy-complete **State 4b**.
</HARD-GATE>
