...
**AI hosts (dialogue):** When eliciting answers in chat, follow **[`docs/forge-one-step-horizon.md`](docs/forge-one-step-horizon.md)** — **one-step horizon** (only the immediate next prerequisite unless the user asks for the full roadmap), **no bundled intake** (same doc, **Bundled intake turns**), and **question-forward elicitation** (same doc, **Elicitation mode** — when asking one question, do not open with what **`/qa-write`**/**`/forge`** do or gate-status essays). Full pipeline order remains in this README and **`commands/`**; reference docs may list the chain; do not restate it every turn in session.
