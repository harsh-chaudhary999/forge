...
| "I'll use **## What unlocks eval YAML** / eval **`*.yaml`** / Step −1 as the **chat title** while **`manual-test-cases.csv`** is still the blocker" | **Invalid headline.** **`docs/forge-one-step-horizon.md`** **Headline = immediate next step**. The **first** heading must match **Step −1 first failing row** — typically **`qa-manual-test-cases-from-prd`** / CSV, **not** YAML. |
...
| "I'll trigger **`AskQuestion`** for task-id / **prd-locked** **and** paste **Q1** in the same assistant message" | **Invalid — dual prompt.** **`docs/forge-one-step-horizon.md`** **No bundled unrelated decisions** (Cursor example). Prerequisite turn **only** → **next** turn **only** Q1. |
...
**General rule (all phases, all hosts):** This is the **QA → machine-eval artifact** slice of **`using-forge`** **Stage-local questioning** — after manual CSV (or approved waiver), choose **either** **`eval/*.yaml`** via this skill **or** the **semantic** path (**`qa-semantic-csv-orchestrate`**, **`docs/semantic-eval-csv.md`**) — never front-load a **blocking interactive prompt** about **later** pipeline steps while **earlier** prerequisites are missing.
...
| 4 | **Machine-eval artifact (pick one path)** | **YAML (standard):** **`qa-write-scenarios`** → **`~/forge/brain/prds/<task-id>/eval/*.yaml`** per **`eval-scenario-format`**. | **Semantic** (when honest YAML would be fiction — no stack to explore yet, NL-first): **`qa-semantic-csv-orchestrate`** + **`docs/semantic-eval-csv.md`** → **`qa/semantic-automation.csv`**, **`qa/semantic-eval-manifest.json`**, **`[P4.0-SEMANTIC-EVAL]`**. Same Step −1 **CSV/waiver** prerequisites above apply **before** either path. |

**Assistant-led messaging (repo-wide — HARD-NORM):** In **chat**, the **title / first paragraph / top `##`** must name **only** the **first failing row** above — e.g. *Next: **`qa-manual-test-cases-from-prd`** / **`manual-test-cases.csv`*** when row **3** fails; after CSV exists, row **4** may require **`eval/*.yaml`** **or** semantic artifacts — never YAML-first **headlines** while rows **1–3** fail. **Do not** open with **`## What unlocks eval YAML`**, *Per Step −1… eval `*.yaml`* **as the main heading**, or a **YAML-first** essay when **`manual-test-cases.csv`** is the blocker; Step −1 is **documentation order**, not the **H1** of the message. **Allowed:** one **subordinate** sentence after the CSV call-to-action (*after CSV approval, eval YAML or the semantic path follows*). **`docs/forge-one-step-horizon.md`** — **Headline = immediate next step**.
...
**Assistant-facing venue:** The waiver wording below is **only** for **this** skill when Step 0.0 fails or the user asks for YAML-before-CSV. **Do not** reuse it as filler during **`qa-prd-analysis`** Step 0.5 — that confuses humans and contradicts **`docs/forge-one-step-horizon.md`**.
