...
**Optional machine layer (teams):** Run **`python3 tools/verify_forge_task.py --task-id <id> --brain ~/forge/brain`** in CI or pre-push on the **brain** repo so missing **`eval/*.yaml`** or bad **`conductor.log`** ordering fails the build — see **`docs/forge-task-verification.md`**. Add **`--strict-tech-plans`** once **`tech-plans/*.md`** exist to fail **`REVIEW_PASS`** without FORGE-GATE Section 0c / recross anchors or misplaced **`### 1b.2a`** (**`tools/verify_tech_plans.py`**). The IDE still does not compile-check sessions; this checks **committed** artifacts.
