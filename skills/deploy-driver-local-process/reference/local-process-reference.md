# local-process deploy driver — reference for `deploy-driver-local-process`

> Progressive-disclosure Level 3 (loaded on demand). Architecture/functions, edge cases, common pitfalls, decision trees, implementation details, usage examples, and operational deep-dives. SKILL.md is the operational contract.
...
- [ ] `[DEPLOY-LOCAL-PROCESS]` marker appended to `conductor.log` with ISO-8601 timestamp.
...
Once `health_check()` confirms the service is responding at the HTTP endpoint, append to conductor.log:
```
YYYY-MM-DDTHH:MM:SSZ [DEPLOY-LOCAL-PROCESS] task_id=<id> target=<host-or-service> outcome=success
```
