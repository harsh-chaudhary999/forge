...
- **HARD-GATE — refuse silently dangerous dispatches:** If the conductor dispatch is for **feature / UI / non-test-only** work and there is no valid **`~/forge/brain/prds/<task-id>/qa/semantic-eval-manifest.json`** (see **`docs/forge-task-verification.md`**), **or** the handoff does not cite a logged **`[P4.0-SEMANTIC-EVAL]`** line, return **`BLOCKED_ORCHESTRATION`** to the conductor — **do not** write production code until State 4b is complete (`conductor-orchestrate` State 4b). When **`forge-product.md`** sets **`forge_qa_csv_before_eval: true`** (including when a **full `/forge`** run persisted **`true`** per **`commands/forge.md`**), also require **`~/forge/brain/prds/<task-id>/qa/manual-test-cases.csv`** with **≥1** row and a logged **`[P4.0-QA-CSV]`** before feature code. Test-only RED tasks from the same phase are OK if the prompt explicitly says **tests-first / TDD RED** only. **Teams:** run **`tools/verify_forge_task.py`** on the brain in CI so the same gates are machine-checked on commit (`docs/forge-task-verification.md`).
