...
description: "WHEN: A PR or diff is ready for review and must be audited against Forge's own project rules (AGENTS.md, AGENTS.md, skill format constraints). Runs AFTER spec-reviewer and code-quality-reviewer as the final enforcement gate."
type: rigid
requires: [brain-read]
version: 1.0.0
preamble-tier: 3
...
Audits a code diff **exclusively against rules the Forge project wrote down**. Not generic best practices. Not style preferences. Only rules that appear in `AGENTS.md`, `AGENTS.md`, or documented skill/agent format constraints.
...
**Enforcement (MUST):**
1. MUST trace every finding to a specific rule in AGENTS.md, AGENTS.md, or skill format docs
...
**Why This Fails:** Rules evolve. AGENTS.md constraint list grows each sprint. A rule you remember from three sessions ago may have been updated, removed, or superseded. Memory-based reviews are stale reviews.

**Enforcement (MUST):**
1. MUST read `AGENTS.md` and `AGENTS.md` fresh at the start of every review session
...
```bash
# Find AGENTS.md files in ancestor directories (project rules)
...
**Core Forge constraints to always check (from AGENTS.md):**
...
Rule: "<exact quoted rule text from AGENTS.md or format doc>"
...
Rule: "No LangChain-style agent frameworks in Forge plugin code." (AGENTS.md)
...
**Standards read:** [AGENTS.md, AGENTS.md, forge-skill-anatomy, etc.]
...
### Edge Case 1: AGENTS.md Not Found

**Symptom:** `find . -name "AGENTS.md"` returns empty. No authoritative rule source.

**Do NOT:** Invent rules from memory or proceed without rules.

**Action:**
1. Check parent directories up to home
2. Check `~/forge/AGENTS.md` explicitly
3. If not found: emit `NEEDS_CONTEXT — Cannot locate AGENTS.md. Standards review cannot proceed without authoritative rule source.`
...
### Edge Case 4: Rule in AGENTS.md Is Ambiguous
...
- [ ] All constraint IDs from AGENTS.md checked against diff
