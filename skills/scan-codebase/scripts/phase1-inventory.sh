...
| sort > ${FORGE_SCAN_TMP}/forge_scan_source_files.txt || true

find "$REPO" -type f \( \
  -name "*.spec.*" -o -name "*.test.*" -o -name "*_test.*" -o -name "test_*.py" \
\) \
| grep -v node_modules | grep -v "\.git/" | grep -v dist \
| sort > ${FORGE_SCAN_TMP}/forge_scan_test_files.txt || true

echo "[1.1] Source files: $(wc -l < "${FORGE_SCAN_TMP}/forge_scan_source_files.txt") | Test files: $(wc -l < "${FORGE_SCAN_TMP}/forge_scan_test_files.txt")"
forge_scan_log_stat "phase=1.1 source_files=$(wc -l < "${FORGE_SCAN_TMP}/forge_scan_source_files.txt") test_files=$(wc -l < "${FORGE_SCAN_TMP}/forge_scan_test_files.txt")"
...
while IFS= read -r file; do
  ...
  head -50 "$file" | grep -E \
    ...
    2>/dev/null || true
done < "${FORGE_SCAN_TMP}/forge_scan_source_files.txt" > ${FORGE_SCAN_TMP}/forge_scan_imports.txt

_import_file_count=$(grep -c "^===" ${FORGE_SCAN_TMP}/forge_scan_imports.txt 2>/dev/null || true)
...
SOURCE_COUNT=$(wc -l < "${FORGE_SCAN_TMP}/forge_scan_source_files.txt")
echo "[1.4] Computing hub scores for $SOURCE_COUNT files (single-pass import analysis)..."

> "${FORGE_SCAN_TMP}/forge_scan_hub_scores.txt"

set +o pipefail
while IFS= read -r file; do
  basename_no_ext=$(basename "$file" | sed 's/\.[^.]*$//')
  # Count import lines that reference this module name
  count=$(grep -c "$basename_no_ext" ${FORGE_SCAN_TMP}/forge_scan_imports.txt 2>/dev/null || true)
  count=${count:-0}
  echo "$count $file"
done < "${FORGE_SCAN_TMP}/forge_scan_source_files.txt" \
| sort -rn > ${FORGE_SCAN_TMP}/forge_scan_hub_scores.txt
set -o pipefail

awk '$1 >= 5 {print $2}' ${FORGE_SCAN_TMP}/forge_scan_hub_scores.txt > ${FORGE_SCAN_TMP}/forge_scan_tier1.txt
awk '$1 >= 3 && $1 < 5 {print $2}' ${FORGE_SCAN_TMP}/forge_scan_hub_scores.txt > ${FORGE_SCAN_TMP}/forge_scan_tier2.txt

echo "[1.4] Tier 1 hubs (5+ refs): $(wc -l < "${FORGE_SCAN_TMP}/forge_scan_tier1.txt") | Tier 2 hubs (3-4 refs): $(wc -l < "${FORGE_SCAN_TMP}/forge_scan_tier2.txt")"
echo "[1.4] Top 10 hubs:"
head -10 ${FORGE_SCAN_TMP}/forge_scan_hub_scores.txt | sed 's/^/  /'
forge_scan_log_stat "phase=1.4 tier1=$(wc -l < "${FORGE_SCAN_TMP}/forge_scan_tier1.txt") tier2=$(wc -l < "${FORGE_SCAN_TMP}/forge_scan_tier2.txt") source_files_scored=$SOURCE_COUNT"
...
TS_COUNT=$(grep -c "\.ts$\|\.tsx$" ${FORGE_SCAN_TMP}/forge_scan_source_files.txt 2>/dev/null || true)
JS_COUNT=$(grep -c "\.js$\|\.jsx$" ${FORGE_SCAN_TMP}/forge_scan_source_files.txt 2>/dev/null || true)
PY_COUNT=$(grep -c "\.py$"  ${FORGE_SCAN_TMP}/forge_scan_source_files.txt 2>/dev/null || true)
GO_COUNT=$(grep -c "\.go$"  ${FORGE_SCAN_TMP}/forge_scan_source_files.txt 2>/dev/null || true)
JAVA_COUNT=$(grep -c "\.java$" ${FORGE_SCAN_TMP}/forge_scan_source_files.txt 2>/dev/null || true)
KT_COUNT=$(grep -c "\.kt$"  ${FORGE_SCAN_TMP}/forge_scan_source_files.txt 2>/dev/null || true)
DART_COUNT=$(grep -c "\.dart$" ${FORGE_SCAN_TMP}/forge_scan_source_files.txt 2>/dev/null || true)
RS_COUNT=$(grep -c "\.rs$"  ${FORGE_SCAN_TMP}/forge_scan_source_files.txt 2>/dev/null || true)
TS_COUNT=${TS_COUNT:-0}
JS_COUNT=${JS_COUNT:-0}
PY_COUNT=${PY_COUNT:-0}
GO_COUNT=${GO_COUNT:-0}
...
if [ "$JAVA_COUNT" -gt 0 ]; then
  grep -rn \
    ...
    2>/dev/null > ${FORGE_SCAN_TMP}/forge_scan_types_java.txt || true
  ...
  grep -rn \
    ...
    "$REPO" --include="*.java" \
    | grep -v "/test/" \
    2>/dev/null > ${FORGE_SCAN_TMP}/forge_scan_annotations_java.txt || true
  ...
  grep -rn \
    ...
    2>/dev/null > ${FORGE_SCAN_TMP}/forge_scan_methods_java.txt || true

  echo "  Java    — types: $(wc -l < "${FORGE_SCAN_TMP}/forge_scan_types_java.txt") | methods: $(wc -l < "${FORGE_SCAN_TMP}/forge_scan_methods_java.txt") | annotations: $(wc -l < "${FORGE_SCAN_TMP}/forge_scan_annotations_java.txt")"
else
  > ${FORGE_SCAN_TMP}/forge_scan_types_java.txt
  > ${FORGE_SCAN_TMP}/forge_scan_methods_java.txt
  > ${FORGE_SCAN_TMP}/forge_scan_annotations_java.txt
fi
...
if [ "$KT_COUNT" -gt 0 ]; then
  grep -rn \
    ...
    2>/dev/null > ${FORGE_SCAN_TMP}/forge_scan_types_kotlin.txt || true

  grep -rn \
    "^\s*@\(Service\|Repository\|Controller\|RestController\|Component\|Configuration\|Entity\|SpringBootApplication\)" \
    "$REPO" --include="*.kt" \
    | grep -v "/test/" \
    2>/dev/null > ${FORGE_SCAN_TMP}/forge_scan_annotations_kotlin.txt || true

  grep -rn \
    "^\s*\(override\s\+\)\?\(suspend\s\+\)\?\(inline\s\+\)\?\(private\s\+\|protected\s\+\|internal\s\+\|public\s\+\)\?\(open\s\+\)\?fun [a-zA-Z_]" \
    "$REPO" --include="*.kt" \
    | grep -v "Test\.kt\b\|Spec\.kt\b\|/test/" \
    2>/dev/null > ${FORGE_SCAN_TMP}/forge_scan_methods_kotlin.txt || true

  echo "  Kotlin  — types: $(wc -l < "${FORGE_SCAN_TMP}/forge_scan_types_kotlin.txt") | functions: $(wc -l < "${FORGE_SCAN_TMP}/forge_scan_methods_kotlin.txt")"
else
  > ${FORGE_SCAN_TMP}/forge_scan_types_kotlin.txt
  > ${FORGE_SCAN_TMP}/forge_scan_methods_kotlin.txt
  > ${FORGE_SCAN_TMP}/forge_scan_annotations_kotlin.txt
fi
...
if [ "$GO_COUNT" -gt 0 ]; then
  grep -rn "^type [A-Z][a-zA-Z0-9]* \(struct\|interface\)\b" \
    "$REPO" --include="*.go" \
    | grep -v "_test\.go" \
    2>/dev/null > ${FORGE_SCAN_TMP}/forge_scan_types_go.txt || true

  grep -rn "^func ([a-zA-Z_][a-zA-Z0-9_]* \*\?[A-Z][a-zA-Z0-9]*) [A-Za-z]" \
    "$REPO" --include="*.go" \
    | grep -v "_test\.go" \
    2>/dev/null > ${FORGE_SCAN_TMP}/forge_scan_methods_go.txt || true

  grep -rn "^func [A-Z][a-zA-Z0-9]*(" \
    "$REPO" --include="*.go" \
    | grep -v "_test\.go" \
    2>/dev/null > ${FORGE_SCAN_TMP}/forge_scan_functions_go.txt || true

  echo "  Go      — types: $(wc -l < "${FORGE_SCAN_TMP}/forge_scan_types_go.txt") | receiver methods: $(wc -l < "${FORGE_SCAN_TMP}/forge_scan_methods_go.txt") | exported funcs: $(wc -l < "${FORGE_SCAN_TMP}/forge_scan_functions_go.txt")"
else
  > ${FORGE_SCAN_TMP}/forge_scan_types_go.txt
  > ${FORGE_SCAN_TMP}/forge_scan_methods_go.txt
  > ${FORGE_SCAN_TMP}/forge_scan_functions_go.txt
fi
...
if [ "$TSJS_COUNT" -gt 0 ]; then
  grep -rn \
    ...
    2>/dev/null > ${FORGE_SCAN_TMP}/forge_scan_types_ts.txt || true
  ...
  grep -rn \
    ...
    2>/dev/null > ${FORGE_SCAN_TMP}/forge_scan_methods_ts.txt || true
  ...
  grep -rn \
    ...
    2>/dev/null > ${FORGE_SCAN_TMP}/forge_scan_decorators_ts.txt || true

  grep -rn "^export \(async \)\?function [a-zA-Z]" \
    "$REPO" --include="*.ts" --include="*.tsx" --include="*.js" --include="*.jsx" \
    | grep -v "node_modules\|\.spec\.\|\.test\." \
    2>/dev/null > ${FORGE_SCAN_TMP}/forge_scan_functions_ts.txt || true
  ...
  echo "  TS/JS   — classes: $(wc -l < "${FORGE_SCAN_TMP}/forge_scan_types_ts.txt") | class methods: $(wc -l < "${FORGE_SCAN_TMP}/forge_scan_methods_ts.txt") | functions: $(wc -l < "${FORGE_SCAN_TMP}/forge_scan_functions_ts.txt") | decorators: $(wc -l < "${FORGE_SCAN_TMP}/forge_scan_decorators_ts.txt")"
else
  > ${FORGE_SCAN_TMP}/forge_scan_types_ts.txt
  > ${FORGE_SCAN_TMP}/forge_scan_methods_ts.txt
  > ${FORGE_SCAN_TMP}/forge_scan_functions_ts.txt
  > ${FORGE_SCAN_TMP}/forge_scan_decorators_ts.txt
fi
...
if [ "$PY_COUNT" -gt 0 ]; then
  grep -rn "^class [A-Za-z][a-zA-Z0-9]*\b" \
    ...
    2>/dev/null > ${FORGE_SCAN_TMP}/forge_scan_types_python.txt || true

  grep -rn \
    "^@\(dataclass\|dataclasses\.dataclass\|property\|staticmethod\|classmethod\|abstractmethod\|app\.route\|router\.\)" \
    "$REPO" --include="*.py" \
    | grep -v "test_\|_test\.py" \
    2>/dev/null > ${FORGE_SCAN_TMP}/forge_scan_annotations_python.txt || true
  ...
  grep -rn "^def [a-zA-Z][a-zA-Z0-9_]*\|^async def [a-zA-Z][a-zA-Z0-9_]*" \
    ...
    2>/dev/null > ${FORGE_SCAN_TMP}/forge_scan_functions_python.txt || true
  ...
  grep -rn "^\s\+def [a-zA-Z][a-zA-Z0-9_]*\|^\s\+async def [a-zA-Z][a-zA-Z0-9_]*" \
    ...
    2>/dev/null > ${FORGE_SCAN_TMP}/forge_scan_methods_python.txt || true

  echo "  Python  — types: $(wc -l < "${FORGE_SCAN_TMP}/forge_scan_types_python.txt") | class methods: $(wc -l < "${FORGE_SCAN_TMP}/forge_scan_methods_python.txt") | module funcs: $(wc -l < "${FORGE_SCAN_TMP}/forge_scan_functions_python.txt")"
else
  > ${FORGE_SCAN_TMP}/forge_scan_types_python.txt
  > ${FORGE_SCAN_TMP}/forge_scan_methods_python.txt
  > ${FORGE_SCAN_TMP}/forge_scan_functions_python.txt
  > ${FORGE_SCAN_TMP}/forge_scan_annotations_python.txt
fi
...
if [ "$DART_COUNT" -gt 0 ]; then
  grep -rn "^\(abstract \)\?class [A-Z]\|^mixin [A-Z]\|^enum [A-Z]" \
    ...
    2>/dev/null > ${FORGE_SCAN_TMP}/forge_scan_types_dart.txt || true

  grep -rn \
    "^\s*\(Future\|Stream\|void\|bool\|int\|double\|String\|Widget\|[A-Z][a-zA-Z0-9<>?]*\)\s\+[a-z_][a-zA-Z0-9_]*\s*(" \
    "$REPO" --include="*.dart" \
    | grep -v "_test\.dart\|test/" \
    2>/dev/null > ${FORGE_SCAN_TMP}/forge_scan_methods_dart.txt || true

  echo "  Dart    — types: $(wc -l < "${FORGE_SCAN_TMP}/forge_scan_types_dart.txt") | methods: $(wc -l < "${FORGE_SCAN_TMP}/forge_scan_methods_dart.txt")"
else
  > ${FORGE_SCAN_TMP}/forge_scan_types_dart.txt
  > ${FORGE_SCAN_TMP}/forge_scan_methods_dart.txt
fi
...
if [ "$RS_COUNT" -gt 0 ]; then
  grep -rn "^pub \(struct\|enum\|trait\) [A-Z]\|^pub(crate) \(struct\|enum\|trait\) [A-Z]" \
    "$REPO" --include="*.rs" \
    | grep -v "test\b\|#\[test\]" \
    2>/dev/null > ${FORGE_SCAN_TMP}/forge_scan_types_rust.txt || true

  grep -rn "^\s*pub fn [a-zA-Z_]\|^pub fn [a-zA-Z_]\|^pub async fn [a-zA-Z_]" \
    "$REPO" --include="*.rs" \
    | grep -v "#\[test\]\|mod tests" \
    2>/dev/null > ${FORGE_SCAN_TMP}/forge_scan_methods_rust.txt || true

  echo "  Rust    — types: $(wc -l < "${FORGE_SCAN_TMP}/forge_scan_types_rust.txt") | pub fns: $(wc -l < "${FORGE_SCAN_TMP}/forge_scan_methods_rust.txt")"
else
  > ${FORGE_SCAN_TMP}/forge_scan_types_rust.txt
  > ${FORGE_SCAN_TMP}/forge_scan_methods_rust.txt
fi

# ── Frontend / HTML / Templates ───────────────────────────────────────────────
find "$REPO" -type f \( -name "*.html" -o -name "*.htm" \) \
  | grep -v "node_modules\|dist\|build\|\.git\|coverage" \
  | sort > ${FORGE_SCAN_TMP}/forge_scan_html_files.txt || true

find "$REPO" -type f -name "*.vue" \
  | grep -v "node_modules\|dist" \
  | sort > ${FORGE_SCAN_TMP}/forge_scan_vue_files.txt || true

find "$REPO" -type f -name "*.svelte" \
  | grep -v "node_modules\|dist" \
  | sort > ${FORGE_SCAN_TMP}/forge_scan_svelte_files.txt || true

find "$REPO" -type f -name "*.component.html" \
  | grep -v "node_modules\|dist" \
  | sort > ${FORGE_SCAN_TMP}/forge_scan_angular_templates.txt || true

grep -rn "<form\s\+\|<form>" \
  "$REPO" --include="*.html" --include="*.vue" --include="*.svelte" \
  --include="*.tsx" --include="*.jsx" \
  | grep -v "node_modules\|dist" \
  2>/dev/null > ${FORGE_SCAN_TMP}/forge_scan_html_forms.txt || true

grep -rn "id=\"[a-zA-Z][a-zA-Z0-9_-]*\"\|data-[a-z][a-z0-9-]*=" \
  "$REPO" --include="*.html" --include="*.vue" --include="*.svelte" \
  | grep -v "node_modules\|dist" \
  2>/dev/null > ${FORGE_SCAN_TMP}/forge_scan_html_ids.txt || true

echo "  Frontend — HTML: $(wc -l < "${FORGE_SCAN_TMP}/forge_scan_html_files.txt") | Vue: $(wc -l < "${FORGE_SCAN_TMP}/forge_scan_vue_files.txt") | Svelte: $(wc -l < "${FORGE_SCAN_TMP}/forge_scan_svelte_files.txt") | Angular: $(wc -l < "${FORGE_SCAN_TMP}/forge_scan_angular_templates.txt") | Forms: $(wc -l < "${FORGE_SCAN_TMP}/forge_scan_html_forms.txt")"
...
cat \
  ...
  ${FORGE_SCAN_TMP}/forge_scan_types_dart.txt \
  ${FORGE_SCAN_TMP}/forge_scan_types_rust.txt \
  2>/dev/null > ${FORGE_SCAN_TMP}/forge_scan_types_all.txt
...
cat \
  ...
  ${FORGE_SCAN_TMP}/forge_scan_methods_dart.txt \
  ${FORGE_SCAN_TMP}/forge_scan_methods_rust.txt \
  2>/dev/null > ${FORGE_SCAN_TMP}/forge_scan_methods_all.txt

cat \
  ${FORGE_SCAN_TMP}/forge_scan_functions_ts.txt \
  ${FORGE_SCAN_TMP}/forge_scan_functions_go.txt \
  ${FORGE_SCAN_TMP}/forge_scan_functions_python.txt \
  2>/dev/null > ${FORGE_SCAN_TMP}/forge_scan_functions_all.txt

cat \
  ${FORGE_SCAN_TMP}/forge_scan_html_files.txt \
  ${FORGE_SCAN_TMP}/forge_scan_vue_files.txt \
  ${FORGE_SCAN_TMP}/forge_scan_svelte_files.txt \
  ${FORGE_SCAN_TMP}/forge_scan_angular_templates.txt \
  2>/dev/null > ${FORGE_SCAN_TMP}/forge_scan_ui_all.txt

TYPES_COUNT=$(wc -l < "${FORGE_SCAN_TMP}/forge_scan_types_all.txt")
METHODS_COUNT=$(wc -l < "${FORGE_SCAN_TMP}/forge_scan_methods_all.txt")
FUNCS_COUNT=$(wc -l < "${FORGE_SCAN_TMP}/forge_scan_functions_all.txt")
UI_COUNT=$(wc -l < "${FORGE_SCAN_TMP}/forge_scan_ui_all.txt")
TOTAL=$(( TYPES_COUNT + METHODS_COUNT + FUNCS_COUNT + UI_COUNT ))

forge_scan_log_stat "phase=1.6 types=$TYPES_COUNT methods=$METHODS_COUNT functions=$FUNCS_COUNT ui=$UI_COUNT html_forms=$(wc -l < "${FORGE_SCAN_TMP}/forge_scan_html_forms.txt") total_potential_nodes=$TOTAL"
...
echo "  HTML forms found:          $(wc -l < "${FORGE_SCAN_TMP}/forge_scan_html_forms.txt")"
echo "══════════════════════════════════════════════════════════"
echo "TOTAL POTENTIAL NODES: $TOTAL"
echo "══════════════════════════════════════════════════════════"
echo ""
echo "Tier 1 hubs (5+ refs): $(wc -l < "${FORGE_SCAN_TMP}/forge_scan_tier1.txt")"
echo "Tier 2 hubs (3-4 refs): $(wc -l < "${FORGE_SCAN_TMP}/forge_scan_tier2.txt")"
echo ""
echo "Phase 1 complete. All inventory files written to ${FORGE_SCAN_TMP}/forge_scan_*.txt"
echo "Next: Phase 2 (hub assignment already done above), Phase 3 (hub reads)"
forge_scan_log_done "tier1=$(wc -l < "${FORGE_SCAN_TMP}/forge_scan_tier1.txt") tier2=$(wc -l < "${FORGE_SCAN_TMP}/forge_scan_tier2.txt") types=$TYPES_COUNT methods=$METHODS_COUNT functions=$FUNCS_COUNT ui=$UI_COUNT total_potential_nodes=$TOTAL"
