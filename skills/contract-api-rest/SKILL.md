...
This skill declares `requires: [brain-read]` — exercise it. Before proposing the REST contract: `brain_recall`/grep the product topology (`products/<slug>/forge-product.md`) and any existing `api-contract.md` + prior API decisions for the affected entity, so this contract supersedes rather than duplicates prior locks. Record the resulting `contract_id` (brain decision id / commit SHA) in the LOCK checklist.
