...
The [Agent Skills standard](https://platform.Codex.com/docs/en/agents-and-tools/agent-skills/overview)
...
### CSO (Codex Search Optimization) for Descriptions
...
1. States that **`AskUserQuestion`** is **canonical** for Codex + **`tools/lint_skill_allowed_tools.py`** — **do not** rename per IDE in **`SKILL.md`**.
...
All skills live in `skills/{skill-name}/SKILL.md` at repo root. The `.Codex/skills/` path is a symlink — never create skills there directly.
