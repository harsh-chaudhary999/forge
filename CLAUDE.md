...
- **Question-forward elicitation:** If you need a **single** answer (e.g. Q9 confirm), do **not** open with what **`/qa-write`** or **`/forge`** “does” or which **gates** are open — **`docs/forge-one-step-horizon.md`** **Elicitation mode**; **`using-forge`** item **7** — unless the user asked for status.
