...
description: "Host install health — plugin trees, Codex hooks symlink, settings.json forge hook counts, Cursor global rules freshness. Read-only."
---
...
Interprets output: **`OK`** lines pass; **`WARN`** means re-run **`bash scripts/install.sh`** for your IDE (`--platform cursor` / `--platform Codex`); **`ERROR`** from the nested verifier means broken plugin layout.
