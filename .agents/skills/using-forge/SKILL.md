---
name: using-forge
description: "Bootstrap skill — inlined by session-start hook for every Forge-supported host (Codex, Cursor, Gemini CLI, JetBrains AI, Codex, Copilot CLI, IDX, Antigravity, OpenCode, etc.)"
type: rigid
version: 1.0.22
preamble-tier: 4
triggers:
...
Rigid skills declare the canonical tool name **`AskUserQuestion`** in **`allowed-tools`** (Codex policy + repo lint). **Semantics are host-agnostic:** every supported Forge IDE must obtain human answers through a **blocking interactive prompt** — never prose-only handoffs.

| Host / surface | Use this for discrete choices |
|----------------|-------------------------------|
| **Codex** | **`AskUserQuestion`** (matches skill `allowed-tools`) |
...
- Same **where / how** as above; plus **forbidden** in this phase: full Q1–Q8 wall in one message; **Q1 in markdown + unrelated `AskQuestion`** (e.g. task-id / **prd-locked** approve) in the **same** turn — **dual prompt** (**`docs/forge-one-step-horizon.md`**); **full Qn text in chat + identical full Qn in `AskQuestion`** — **deduplicate** (**short** widget title + options **only**); “single bulk / approve all” shortcuts; **CSV / automation-only waiver** choices (**wrong gate** — use **`qa-manual-test-cases-from-prd`** / **`/qa-write`** **after** `qa-analysis.md`). **Q1** must show the **full** test-type checklist (not preset-only **Full/Lean** — see **`qa-prd-analysis`** Q1 **HARD-GATE**).
...
1. **User's explicit instructions** (AGENTS.md, user direct requests) — highest priority
...
**Codex `session-start` hook (stage + preamble):** Optional env vars (export in shell or IDE env before launching Codex):
...
**Optional local skill lock (Codex hooks):**
...
Stage resolution uses the **last** `[P…]` marker in the chosen log; implementation: **`.Codex/hooks/forge-stage-detect.cjs`**. Run **`node .Codex/hooks/test-forge-stage-detect.cjs`** (from repo root) to verify mapping after edits.
...
- **Subagents:** `~/.Codex/agents/<agent-name>.md`
