...
| `/qa` | [qa-pipeline-orchestrate](../skills/qa-pipeline-orchestrate/SKILL.md) (chains downstream) | **Read**; may add terms if analysis finds new entities | **`commands/qa-write.md`** ordering; **Phase QA-P7** optional `terminology_*` in run report (see skill); same dialogue norms as [forge-one-step-horizon](forge-one-step-horizon.md) |
| `/qa-write` | [qa-semantic-csv-orchestrate](../skills/qa-semantic-csv-orchestrate/SKILL.md) | **Read** before bulk **`qa/semantic-automation.csv`** | **Warn** if missing; canonical step text; see **`docs/semantic-eval-csv.md`** |
...
|-------|------|------------------|-----------------|
| [qa-pipeline-orchestrate](../skills/qa-pipeline-orchestrate/SKILL.md) | Standalone QA-P1…P7 | **QA-P1** read; **QA-P7** optional report frontmatter | This doc **§ Entrypoint matrix** + `using-forge` **Stage-local questioning**; do **not** **blocking-prompt** CSV/automation waivers before **`commands/qa-write.md`** prerequisites |
| [qa-semantic-csv-orchestrate](../skills/qa-semantic-csv-orchestrate/SKILL.md) | Writes **`qa/semantic-automation.csv`** + manifest | **Step 0** `cat`; schema per **`docs/semantic-eval-csv.md`** | Ordering per **`commands/qa-write.md`**; [forge-glossary](../skills/forge-glossary/SKILL.md) ≠ product terms |
