...
```bash
HOOK="$REPO_ROOT/.Codex/hooks/session-start.cjs"
if [ -f "$HOOK" ]; then
  echo "✓ Session-start hook  .Codex/hooks/session-start.cjs — present"
else
  echo "✗ Session-start hook  .Codex/hooks/session-start.cjs — NOT FOUND"
  echo "  Fix: verify .Codex/hooks/ directory contains session-start.cjs"
fi
```
...
```bash
HOOK2="$REPO_ROOT/.Codex/hooks/pre-tool-use.cjs"
if [ -f "$HOOK2" ]; then
  echo "✓ Pre-tool-use hook   .Codex/hooks/pre-tool-use.cjs — present"
else
  echo "✗ Pre-tool-use hook   .Codex/hooks/pre-tool-use.cjs — NOT FOUND"
  echo "  Fix: verify .Codex/hooks/ directory contains pre-tool-use.cjs"
fi
```
