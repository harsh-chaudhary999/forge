...
Use when eval/QA work discovers a **mismatch** between `expected` text in **`eval/*.yaml`**, the running app, and `terminology.md` **before** you are ready to edit the canonical term sheet. Each row: **date**, **source** (file + id), **observed vs canonical**, **resolution** (link to `terminology.md` revision or pending).
