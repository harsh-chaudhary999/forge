...
3. **Claude Code** (`pre-tool-use.cjs`): when **`~/.forge/.active-skill`** is set, **`hooks/hooks.json`** should register **PreToolUse** for every tool name your host emits (see matcher in that file — alternation of tool names). Then the hook can enforce **`allowed_tools`** from **`tools/dev/skill-tool-policy.json`** (with fallback **`tools/skill-tool-policy.json`**) or **`skills/<name>/SKILL.md`**. **`FORGE_ROOT`** may point at a non-default Forge checkout. **HARD-GATE** entries **deny** disallowed tools; others **ask**. Canary and destructive-pattern checks remain **Bash-only**. **`session-start.cjs`** optionally reads **`~/.forge/.active-skill-tier`** (single line **`1`–`4`**, aligned with **`preamble-tier`** in **`SKILL.md`**) for preamble injection; if **`active-skill`** is set but **`active-skill-tier`** is missing, session-start parses **`SKILL.md`** once and writes **`~/.forge/.active-skill-tier`** for subsequent fast loads — operators may **`echo N > ~/.forge/.active-skill-tier`** when setting **`active-skill`** by hand. Other IDEs need their own wiring if you want the same behavior.
