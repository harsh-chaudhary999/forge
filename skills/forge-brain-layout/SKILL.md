...
See [reference/file-formats.md](reference/file-formats.md) for the full file format specs — Decision, Draft, Archived Decision, and PRD (Product Context) file formats with frontmatter and section templates.
...
Full how-not-to catalog (with the right alternative for each) in [reference/anti-patterns.md](reference/anti-patterns.md).

## Edge Cases: How to Handle Unusual Situations

See [reference/edge-cases.md](reference/edge-cases.md) for full symptom → root cause → action plan → escalation for each. Summary of the three covered situations:

| # | Edge case | Symptom (one-line) | Escalation |
|---|---|---|---|
| 1 | Brain Not Initialized | `brain/` directory doesn't exist or is not in git | NEEDS_INFRA_CHANGE |
| 2 | Brain Corrupted or Lost | Brain exists but not tracked / wrong perms / partial structure | NEEDS_COORDINATION |
| 3 | Brain in Wrong Directory | Wrong branch/repo cloned, unexpected path, or multiple brains | NEEDS_CONTEXT |
...
See [reference/navigation-and-query-patterns.md](reference/navigation-and-query-patterns.md) for the exhaustive catalog — navigate by product area, by decision category, by decision status, by keyword/relationship, plus the specific copy-paste query patterns (eval results, skill candidates, cross-references, latest decisions, keyword grep, recently archived). The Quick Reference Card and Common Queries table above cover the day-to-day paths.
