...
**Human input (all hosts):** **`AskUserQuestion`** in **`allowed-tools`** is canonical for Codex + skill lint. Map to the host blocking prompt per **`skills/using-forge/SKILL.md`** (Cursor **`AskQuestion`**; numbered list + stop if unavailable).
...
**HARD-GATE (host):** Before wiring **real** CDP, ADB, or MCP execution, ask the operator **MCP vs local driver** (and for mobile, **Appium MCP vs ADB vs XCTest**) per **AGENTS.md** D5 — record the choice in the task brain.
