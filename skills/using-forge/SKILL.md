...
### Interactive human input — project standard (all doubts / choices)
...
1. Prefer **`AskUserQuestion`** with explicit options when the skill allows or the decision fits discrete choices.
2. If the decision is **not** a discrete choice (open-ended / free text): **numbered options** in the assistant message **plus** **stop and wait**.
3. When more than one human answer is needed across a workflow phase, follow **Multi-question elicitation** below (skill-specific **what** to ask stays in each **`SKILL.md`**). **Exceptions:** verbatim one-shot gates (**`intake-interrogate` Q9** blockquote — still **chat-visible** before relying on **`AskUserQuestion`** alone).

**Forbidden as the *only* way to decide:**

- Long **“What to do next”** / **runbook** sections (e.g. *pick task-id → /intake → qa-prd-analysis → CSV → /qa-write → …*) that end with *“if you reply with task-id / confirm you’ve started intake…”* **without** a same-turn **blocking interactive prompt** or **numbered list** the user can click or answer in one step. **That is not interactive** — it is documentation disguised as a question. The user did not get **`AskUserQuestion`** or **numbered options + stop**.
...
- **Chat vs widget deduplication (same fork):** The **long** context (Q4 feature list, Q1 checklist, etc.) belongs **in chat** **once**. The **`AskUserQuestion`** widget should pass a **short** title + **discrete options** — **not** a **verbatim** second copy of the entire same prompt (**`docs/forge-one-step-horizon.md`** **Chat vs `AskUserQuestion` widget**). Reading the **same** paragraph in **both** panes is invalid UX; **empty** chat + **only** modal still fails transcript-first.
- **Headline = immediate next artifact (message structure):** The **first** **`##` heading**, bold lead, or opening sentence must name the **single next** gate from the active skill’s ordering table (**e.g.** **`/qa-write` ordering** **first failing row**) — usually **`manual-test-cases.csv`** / **`qa-manual-test-cases-from-prd`** or **`qa-prd-analysis`**, **not** semantic machine eval when CSV is still open. **Forbidden:** **`## What unlocks machine eval`** or the full ordering table **as the chat title** when the blocker is **manual CSV**. Ordering prose lives in **`SKILL.md`** for reference; the **headline** follows **first failing row**. **`docs/forge-one-step-horizon.md`** — **Headline = immediate next step**.

**How (dialogue mechanics — skill-agnostic):**

1. **One primary dimension per assistant message** when multiple questions remain — not Q1–Q8 in one turn. **A single dimension may include a long structured checklist** (e.g. **`qa-prd-analysis` Q1** full test-type menu) — that is still **one** topic; do **not** collapse it to presets-only **without** showing the full menu when the canonical skill template is a checklist (**see `qa-prd-analysis` Q1 HARD-GATE**). **In that same turn, do not** also fire **`AskUserQuestion`** for a **different** topic (e.g. **task-id** / **prd-locked** approval while Q1 is in the message body) — **two competing primaries**; **sequence** prerequisite **→** **then** Q1 on the **next** message (**`docs/forge-one-step-horizon.md`** **No bundled unrelated decisions**).
2. **Blocking interactive** for discrete forks: **`AskUserQuestion`** (or **numbered 1–N + stop** for open-ended forks) per **Blocking interactive prompts** above.
...
6. **No bundled unrelated decisions:** Do **not** use **one** **`AskUserQuestion`** (e.g. task-id **A vs B**) as the **only** blocking affordance while demanding **other** needle-moving answers **only** in **prose** in the **same** message. **Each** discrete fork needs **its own** turn with **`AskUserQuestion`** / **numbered options + stop**, or **strict sequential** follow-ups — not a prose *“Reply with (a) and (b)…”* wall. **Do not** paste **phase-specific** waiver or ordering copy from a **later** skill’s gate while the human is still in an **earlier** phase — **`docs/forge-one-step-horizon.md`** **No bundled unrelated decisions** and **Phase-specific waivers (example)**.
7. **Question-forward elicitation:** When the **purpose** of the message is to obtain **one** human answer, **do not** prefix with a tutorial on what **`commands/`**, **`README`**, or **named skills** do, which **gates** are open, or which **later** artifacts do not exist yet — and **do not** paste **defensive downstream-gate** essays (*why semantic machine eval / CSV / merge isn’t ready yet*, full ordering-table chains, *orphan automation*) **between** sequential questions; that norm is **repo-wide** (**`docs/forge-one-step-horizon.md`** **No defensive downstream-gate narration (repo-wide)**). The user is in flow; reference docs are for when they choose to read them. **Exceptions:** user asked for status, “what’s blocking,” or the full order; or you are **refusing** a premature skip-ahead (**first missing** prerequisite + next action). **Norm:** **`docs/forge-one-step-horizon.md`** **Question-forward elicitation**.
...
This pattern is **not** QA-specific — it applies to **every** Forge phase.
...
- Same **where / how** as above; plus **forbidden** in this phase: full Q1–Q8 wall in one message; **Q1 in markdown + unrelated `AskUserQuestion`** (e.g. task-id / **prd-locked** approve) in the **same** turn — **dual prompt** (**`docs/forge-one-step-horizon.md`**); **full Qn text in chat + identical full Qn in `AskUserQuestion`** — **deduplicate** (**short** widget title + options **only**); “single bulk / approve all” shortcuts; **CSV / automation-only waiver** choices (**wrong gate** — use **`qa-manual-test-cases-from-prd`** / **`/qa-write`** **after** `qa-analysis.md`). **Q1** must show the **full** test-type checklist (not preset-only **Full/Lean** — see **`qa-prd-analysis`** Q1 **HARD-GATE**).

**Downstream references** to “Step 0.5” or “real interrogation” mean: **`qa-analysis.md`** after **Multi-question elicitation** completed for coverage — **`qa-manual-test-cases-from-prd`**, **`qa-semantic-csv-orchestrate`**, **`qa-pipeline-orchestrate`**, **`conductor-orchestrate`**.
...
3. **First gap wins.** If several prerequisites are missing, surface and fix the **earliest** failure in dependency order (per **`conductor-orchestrate`**, the active skill, or **`commands/qa-write.md`** / **`qa-pipeline-orchestrate`** ordering for the QA→eval slice). Do not **jump ahead** to “how should we proceed on step 5?” when steps **1–4** are not satisfied.
...
5. **Speak only the immediate next dependency.** In **assistant messages** (not static README/docs), **do not** mention downstream process steps unless **(i)** they are the **very next** artifact/skill after the current step, **(ii)** the **current** prompt cannot be answered without naming them, or **(iii)** the user explicitly asked what comes later. Listing **`qa/semantic-automation.csv`**, **semantic manifest** paths, CSV waivers, merge order, PR set, etc. **before** the user is on that gate **confuses** and reads like pressure — **forbidden**.
...
|--------------|---------|
| **Tightly coupled (hard for automation)** | Anything that must live **on disk under `~/forge/brain/`** for skills and subagents to read — especially **`prd-locked.md`**, **`qa/qa-analysis.md`**, and **`manual-test-cases.csv`** (or documented waiver) **before** bulk machine-eval work (**`qa/semantic-automation.csv`** + **`qa/semantic-eval-manifest.json`** per **`docs/semantic-eval-csv.md`**). Chat and external wiki URLs **alone** are not substitutes; **brain files are the transport layer.** |
| **Loosely coupled (recommended, not ordering-table blockers for `/qa-write`)** | **Council**, **`shared-dev-spec.md`**, **tech plans** — they make automation rows **precise** (routes, contracts, task IDs) but are **not** required to *start* the QA analysis → CSV → semantic automation chain if **`prd-locked`** + **`qa-prd-analysis`** can run. |
...
Full edge-case catalog (ambiguous triggers, missing brain context, host quirks, recovery paths) in [reference/edge-cases.md](reference/edge-cases.md).
...
START: Given a task or question
  ...
  │  │  │       Then: Run qa-semantic-csv-orchestrate (after stack-up when executing)
...
- **Product config:** `~/forge/brain/products/<slug>/forge-product.md` (repos, roles, infra)
...
- **Machine eval:** **`qa/semantic-automation.csv`** + **`qa/semantic-eval-manifest.json`** (+ **`qa/semantic-eval-run.log`** when produced) per **`docs/semantic-eval-csv.md`** — orchestrated by **`qa-semantic-csv-orchestrate`**, **`/qa-write`**, **`/qa`**, **`/qa-run`**. **`scenarios-manifest.md`**, **`branch-env-manifest.md`**, **`qa-run-report-<ts>.md`** come from the standalone QA pipeline when applicable.
...
1. Run `/workspace` to register repos and create `forge-product.md` — scan runs automatically
