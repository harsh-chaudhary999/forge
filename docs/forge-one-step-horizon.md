...
**Scope:** This norm applies to **live assistant / agent messages** in Claude Code — not to static reference docs (`README.md`, `commands/*.md` diagrams, brain templates), which may list full dependency order for verification.
...
**What this is:** Assistant prose whose **primary job** is to explain why a **later** gate or artifact does not exist yet — e.g. *semantic machine eval isn’t ready yet because…*, *orphan automation*, pasting the full **Forge QA→CSV→semantic automation** ordering chain, council → merge previews — **while** the human is still in an **earlier** phase (intake, **`qa-prd-analysis`** Q1–Q8, council rounds, tech-plan elicitation, branch/env prep, …).
...
- **Explicit ask:** The user asked *why isn’t machine eval done yet?*, *what’s the full order?*, *where are we?* — then a concise answer is appropriate.
...
**Problem:** One message presents **one** structured prompt (**`AskUserQuestion`** / numbered choices for **one** fork only) while burying **other mandatory decisions** in prose — or mixes **unrelated** meta-instructions (roadmap, waiver text from another phase) in the same turn.
...
**Example (intake):** **task-id** or slug confirmation **must not** be the **only** **`AskUserQuestion`** while Q9 design authority, Figma locks, and similar **appear only in prose** in the same message — use **sequential** turns per **`intake-interrogate`**.

**Example (QA):** Do not paste **ordering-table** / *why semantic machine eval isn’t ready* essays between Q1→Q2→Q3 — see **No defensive downstream-gate narration (repo-wide)** above.

**Example (`AskUserQuestion` widget + markdown):** When the session shows an **`AskUserQuestion`** widget **and** a **main chat message**, **one turn = one primary question** — not two. **Forbidden:** the message body contains **Q1** (or any full **Step 0.5** coverage block, e.g. the test-type checklist) **while** the widget asks a **different** thing (approve **task-id** + **prd-locked**, “what do you want next?”, draft path). That stacks **two unrelated decisions**; the user cannot tell which to answer first and it reads like the agent **contradicts its own step**. **Required sequence:** **Turn 1** — **only** the prerequisite: **either** the widget **or** short markdown + **numbered options** for *that* fork **alone** (no Q1 in the same message). **Turn 2** — **only** Q1 (full checklist as the skill requires; **no** second widget on another topic). **Turn 3+** — Q2, Q3, … one dimension per message. Same rule for any **intake** / **council** mix: **never** two different **blocking** question surfaces in one assistant turn.

**Chat vs `AskUserQuestion` widget — same fork, no verbatim duplicate:** **Transcript-first** means the **substance** of the turn (feature list, Q1 menu, constraints) must be **visible in the thread** — not **only** in the modal. It does **not** mean pasting the **identical** long question **twice**: once in **markdown** and again in full inside the **`AskUserQuestion`** widget. **Do this instead:** **Chat** = the **full** context (e.g. Q4’s numbered areas, “How should we weight…?”). **Widget** = **short** title (e.g. *Q4 — priority scheme for scenario density*) + **A–E** (or N) **option lines** **only** — **not** a second copy of the whole Q4 essay. The user should **not** read the same block twice. **Still forbidden:** **only** a widget with no chat line (fails audit); at least a **one-line** pointer in chat if the list is **only** in a file the user did not see.

## Headline = immediate next step (not downstream machine eval)

**Problem:** The assistant uses **`## What unlocks machine eval`**, *Per **`/qa-write` ordering*** **`qa/semantic-automation.csv`**, or the **full QA→CSV→semantic automation chain** as the **title**, **`##` heading**, or **first paragraph** of a message — while the user’s **actual** next work is **`qa-manual-test-cases-from-prd`**, **`manual-test-cases.csv`**, or expanding rows — **not** authoring **`qa/semantic-automation.csv`** yet. **Semantic machine eval** is **later** in the ordering table; it must **not** **steal the headline** when it is **not** the **immediate** gate.

**Required:** The **top-of-message** framing (first heading, bold lead, or opening sentence) names the **single next** artifact or skill from the **Forge QA ordering table** **first failing row** — e.g. **Next: complete / expand `qa/manual-test-cases.csv`** (**`qa-manual-test-cases-from-prd`**), or **Next: run `qa-prd-analysis`**. Ordering prose is **reference** text in **`SKILL.md`** / **README** — **not** the **H1** of chat when CSV is still open.

**Forbidden:** Opening with *What unlocks machine eval* / *semantic CSV is gated by…* as the **primary** section when **`manual-test-cases.csv`** is **missing** or **needs work**. **Allowed:** one **subordinate** sentence after the CSV CTA (*after CSV approval, `/qa-write` follows*) — **not** before.

**Relation:** Extends **Horizon narration**, **No defensive downstream-gate narration**, **Stage-local questioning** — the **next step** is often **manual CSV**, not automation rows; narration must match.

## Phase-specific waivers (example)

Some products include **manual QA CSV** before **semantic machine eval**. Instructions about **`csv_baseline_waiver_user_quote`**, “say so explicitly in your own words,” or **automation-before-CSV** waivers belong **only** where the skill that owns that gate says — e.g. **`qa-manual-test-cases-from-prd`** / **`commands/qa-write.md`** when that flow is active and the artifact preconditions match.
...
**Assistant chat:** Follow **`docs/forge-one-step-horizon.md`** and **`skills/using-forge/SKILL.md`** — **one-step horizon**; **question-forward** elicitation (no unsolicited command/skill-reference **preface**, no **later-stage** status **suffix** on single-answer turns, **no defensive downstream-gate narration** mid-elicitation — **No defensive downstream-gate narration (repo-wide)** above); **one blocking affordance per unrelated fork** (no bundled prose obligations); **no dual prompts** — **never** the **`AskUserQuestion`** widget on **one** topic **and** a **long markdown question** on **another** in the **same** message (e.g. task-id approval widget + **Q1** checklist body — **No bundled unrelated decisions**); **no chat–widget duplicate** — long lists / same question body **once** in **chat**; **widget** = **short** title + **options** only (**Chat vs `AskUserQuestion` widget** above); **headline / first § = immediate next artifact** — **not** *What unlocks machine eval*, **`qa/semantic-automation.csv`**, or the full ordering table **as the main heading** when **manual CSV** / **`qa-manual-test-cases-from-prd`** / **`qa-prd-analysis`** is still the next gate (**Headline = immediate next step** above); **phase-specific** waivers/ordering **only** where this doc and the active skill say; **Multi-question elicitation** (items **4–8**) & **Blocking interactive prompts**.

Rigid skills with **`AskUserQuestion`** should add under **Human input:** a **Cross-cutting assistant dialogue** one-liner pointing here + **`using-forge`** items **4–8** (see **`forge-skill-anatomy`**).