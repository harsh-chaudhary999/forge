...
**Before running the Analysis Framework**, check if design assets were provided. Design assets are optional but change what you can produce — with them, you move from spec-derived reasoning to spec-validated reasoning. Follow the priority order (disk → Lovable+GitHub → Figma MCP → Figma REST → human export); if only a bare Figma/wiki URL exists with no implementable assets, **STOP** and return to intake. When design assets are present, extract the component/layout/state inventory, map Design → Contract implications, and add a `## Design Gap Analysis` section to your output.

Full priority order, bare-URL STOP rules, screenshot extraction checklist, Design → Contract table, and Design Gap Analysis in [reference/design-input.md](reference/design-input.md).
...
Define measurable performance targets — Core Web Vitals (LCP, INP, CLS), TTI, initial load, bundle size, JS parse/compile — plus optimization levers (code splitting, image optimization, third-party impact, resource hints, caching).

Full CWV threshold table and optimization considerations in [reference/core-web-vitals.md](reference/core-web-vitals.md).

### 5. Accessibility

Ensure WCAG 2.2 AA compliance (W3C Recommendation since Oct 2023; adds focus-not-obscured, target-size 2.5.8, dragging alternatives, accessible authentication). Cover keyboard navigation, screen reader support, color contrast (4.5:1), focus management, error association, 44x44px touch targets, reduced-motion, and form labeling.

Full WCAG 2.2 AA criteria and accessibility patterns in [reference/accessibility.md](reference/accessibility.md).
...
Full worked example — "Add 2FA to user accounts" rendered as a complete Web Frontend Perspective (pages & components, state shapes, 6 API contracts, performance budget, accessibility, dependencies, risks) in [reference/examples.md](reference/examples.md).

## Common Pitfalls & State Management Patterns

7 common pitfalls (instant-state assumptions, over-engineering, localStorage staleness, late contract mismatches, form-state complexity, async races, prop drilling) with counters, plus the state management decision tree, persistence strategy checklist, and backend sync points in [reference/patterns.md](reference/patterns.md).

## Edge Cases & Handling Strategies

6 edge cases — optimistic-update conflicts, nested form validation, sibling state sharing, large-dataset performance, async race conditions, and localStorage staleness — each with worked code and a flag/alert/proceed escalation path in [reference/edge-cases.md](reference/edge-cases.md).
