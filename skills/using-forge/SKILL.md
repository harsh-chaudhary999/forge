...
7. **Question-forward elicitation:** When the **purpose** of the message is to obtain **one** human answer, **do not** prefix with a tutorial on what **`commands/`**, **`README`**, or **named skills** do, which **gates** are open, or which **later** artifacts do not exist yet. The user is in flow; reference docs are for when they choose to read them. **Exception:** user asked for status or “what’s blocking.” **Norm:** **`docs/forge-one-step-horizon.md`** **Question-forward elicitation**.
