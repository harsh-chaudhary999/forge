...
triggers:
  ...
  - "check worktree before starting"
allowed-tools:
  - Bash
---
...
1. **Create worktree** (invoke `/worktree-per-project-per-task`)
   ...
   - Directory: .Codex/worktrees/TASKID/
...
- **After eval passes on main:**
  ...
  2. Remove worktree directory (.Codex/worktrees/TASKID/)
...
- **Symptom:** "Deployment script requires /opt/myapp, won't work in .Codex/worktrees"
...
- [ ] Worktree directory is isolated (.Codex/worktrees/TASKID/)
...
**Example:** `.Codex/worktrees/` directory is 500GB; 30 old worktrees with node_modules accumulated. Disk has <10GB free.
...
3. Cleanup stale worktrees:
   ```bash
   rm -rf .Codex/worktrees/OLD_TASKID/
   ```
...
**Example:** "Permission denied: .Codex/worktrees/TASKID/" or "Cannot delete, files in use by another process" or ".git/HEAD is locked"
...
2. If files locked:
   - Find process: `lsof +D .Codex/worktrees/TASKID/`
   - Kill process: `kill <PID>`
   - Wait for file descriptors to close
   - Retry cleanup: `rm -rf .Codex/worktrees/TASKID/`
3. If permission issue:
   - Check file ownership: `ls -la .Codex/worktrees/TASKID/`
   - Fix ownership (if running as different user): `chown -R $USER .Codex/worktrees/TASKID/`
   - Retry cleanup
4. If git lock exists:
   - Remove lock: `rm .Codex/worktrees/TASKID/.git/HEAD.lock`
   - Retry cleanup
5. If cleanup still fails:
   - Force delete (use with caution): `rm -rf --force .Codex/worktrees/TASKID/`
