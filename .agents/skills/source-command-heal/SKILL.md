...
Use this skill when the user asks to run the migrated source command `heal`.
...
**Product terms:** When a failure is an **assertion on visible copy** (banner text, label, API error `message`) and **`~/forge/brain/prds/<task-id>/terminology.md`** exists, check for **mismatch** between scenario **`expected`**, app output, and the term sheet; fix **code** or **terminology** with a logged decision — not ad hoc renames in chat only ([docs/terminology-review.md](../docs/terminology-review.md)).
